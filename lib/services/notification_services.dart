...
class NotifyHelper {
  ...
  scheduledNotification(int hour, int minutes, Task task) async {
    debugPrint('hour value is from scheduledNotification $hour');
    debugPrint('minutes value is from scheduledNotification $minutes');
  ...
  tz.TZDateTime _nextInstanceOfTenAM(int hour, int minutes, int remind, String repeat, String date ) {
    ...
    debugPrint('Date from _nextInstanceOfTenAM = $date');
    final tz.TZDateTime localFormattedDate = tz.TZDateTime.from(
        formattedDate,
        tz.local);
    tz.TZDateTime scheduledDate =
        tz.TZDateTime(tz.local, localFormattedDate.year, localFormattedDate.month, localFormattedDate.day, hour, minutes);
    debugPrint('scheduled before reminder = $scheduledDate');
    scheduledDate = afterRemind(remind, scheduledDate);

    if (scheduledDate.isBefore(now)) {
      debugPrint('Time is Before Time = $scheduledDate');
      if(repeat == 'Daily'){
    ...
    debugPrint('Time after edit = $scheduledDate');
  ...
  tz.TZDateTime afterRemind(int remind, tz.TZDateTime scheduledDate) {
     ...
     debugPrint('scheduled after reminder = $scheduledDate');
     return scheduledDate;
  }
 ...
 */
  ...
  void _configureSelectNotificationSubject() {
    selectNotificationSubject.stream.listen((String payload) async {
      await Get.to(() => NotificationScreen(payLode: payload));
    });
  }
}
